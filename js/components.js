...
/* House/roof icon — chimney LEFT, peaked roof above "y" only */
const HOUSE_SVG = `
  <svg viewBox="0 0 34 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    <rect x="5" y="0" width="5" height="8" rx="1" fill="currentColor"/>
    <polyline points="0,21 17,5 34,21" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/>
  </svg>`;

const LOGO_HTML = `
  <a href="index.html" class="nav-logo">
    <span class="logo-wordmark">
      <span class="logo-stay" style="position:relative;display:inline-block">Stay<span style="position:absolute;top:-4px;left:80%;transform:translateX(-50%) scaleX(-1);width:20px;height:13px;line-height:0;display:block;color:inherit">${HOUSE_SVG}</span></span><span class="logo-dekh">Dekh</span>
      <span class="logo-o" style="position:relative;top:4px;display:inline-block">${LOGO_SVG}</span>
    </span>
  </a>`;
...
function injectFloatingCTA() {
  ...
  style.textContent = `
    ...
    /* Attention pulse on the WhatsApp button — a ring expanding out of it.
       Drawn on a pseudo-element so the button itself never changes. */
    .sd-fab-wa::after {
      content: ''; position: absolute; inset: 0; border-radius: 50px;
      border: 2px solid #25D366; opacity: 0; pointer-events: none;
      animation: sdRipple 9s ease-out infinite;
    }
    .sd-fab-wa { position: relative; }
    @keyframes sdRipple {
